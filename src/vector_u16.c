 ...
 */
uint16_t v16_gcd(uint16_t one, uint16_t two){
...
vector_u16_t v16_get_unit_vector(vector_u16_t* vector){
    ...
    uint32_t magnitude = v16_get_magnitude(vector);
...
vector_u16_t v16_get_direction(vector_u16_t* vector){
    ...
    int denominator = v16_gcd(vector->x, vector->y);

    vector_u16_t result = *vector;

    v16_divide(&result, denominator);
